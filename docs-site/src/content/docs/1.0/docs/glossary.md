...
|------|------------------|
| [Trust domain](/1.0/docs/#trust-domain) | Inbound identity plane (`td`) |
| [Audience](/1.0/docs/#audience) | Outbound target (`aud`) |
| [Token exchange](/1.0/docs/#token-exchange) | Credential in → WIMSE JWT out |
| [Revocation](/1.0/docs/#revocation--kill-switch) | Immediate deny for compromised IDs |
| [PEP](/1.0/docs/#pep-policy-enforcement-point) | Starfly at runtime |
| [UTC](/1.0/docs/#utc-universal-tool-calling-layer) | One verifier, many protocols |
...
Deep dive: [trust domains](/1.0/docs/concepts/trust-domains/).
...
- MCP binds `aud` to one tool; using it elsewhere is a [confused deputy](/1.0/docs/integrators/mcp/).
...
Starfly **issues** WIMSE; SPIFFE SVIDs, K8s tokens, and IdP tokens are common **inputs** to exchange — not alternate WIMSE implementations. See [credential patterns](/1.0/docs/integrators/credential-patterns/).

## Token exchange

RFC 8693 at `POST /v1/exchange/token`. Trade a platform credential (K8s SA, OIDC, SPIFFE, stub JWT in dev) for a WIMSE JWT.

Guide: [token exchange integrator](/1.0/docs/integrators/token-exchange/) · Concepts: [exchange](/1.0/docs/concepts/exchange/).
...
Concepts: [revocation](/1.0/docs/concepts/revocation/) · Try: `./sandbox/run.sh revocation`
...
Guide: [MCP security](/1.0/docs/integrators/mcp/) · Code: [`pkg/mcp/`](https://github.com/raygj/project-starfly-fabrics/tree/main/pkg/mcp)
...
Guide: [UTC](/1.0/docs/integrators/utc/) · Code: [`pkg/toolcall/`](https://github.com/raygj/project-starfly-fabrics/tree/main/pkg/toolcall)
...
Guide: [Starfly Graph](/1.0/docs/integrators/starfly-graph/)
...
- [Getting started](/1.0/docs/getting-started/)
