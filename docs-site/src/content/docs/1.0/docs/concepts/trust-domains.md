...
A token with `aud` for tool A must not work at tool B — even when both tools sit in the same trust domain. That is the confused-deputy fix in [MCP security](/1.0/docs/integrators/mcp/).
...
Uses synthetic `dev.local`. Stub JWTs in [getting started](/1.0/docs/getting-started/) exercise exchange without a real IdP.
...
Watch `td` on the issued JWT — [token exchange integrator guide](/1.0/docs/integrators/token-exchange/).

## Related

- [Glossary: trust domain vs audience](/1.0/docs/glossary/#audience)
- [Exchange](/1.0/docs/exchange/)
