...
- **Visible in ops** — denials show on the [MCP Security dashboard tab](/1.0/docs/dashboard/) and in audit.
...
For multi-protocol tool servers, add [UTC](/1.0/docs/utc/) middleware in front of handlers; PEP-side registration and verify stay the same.
...
The `audience` **must** equal the tool's `resource_uri`. See [token exchange](/1.0/docs/token-exchange/) for credential types and fields.
...
Send CAEP `session-revoked` or tool-specific signals via `POST /v1/signals/events`. See [revocation](/1.0/concepts/revocation/).
...
- [Token exchange](/1.0/docs/token-exchange/)
- [UTC](/1.0/docs/utc/) — same identity on non-MCP wire shapes
- [Glossary: MCP](/1.0/glossary/#mcp-model-context-protocol)
