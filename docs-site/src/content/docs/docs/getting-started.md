...
In dev mode, any parseable JWT is accepted against the synthetic `dev.local` [trust domain](/docs/concepts/trust-domains/).
...
Key claims: `sub`, `aud`, `td` (trust domain), `exp`. Deeper dive: [exchange concepts](/docs/concepts/exchange/).
...
These same streams power the [operations dashboard](/docs/integrators/dashboard/) when deployed.
...
| Goal | Go here |
|------|---------|
| Vocabulary | [Glossary](/docs/glossary/) |
| Wire an agent | [Token exchange](/docs/integrators/token-exchange/) |
| MCP tool security | [MCP security](/docs/integrators/mcp/) |
| Multi-protocol tools | [UTC](/docs/integrators/utc/) |
