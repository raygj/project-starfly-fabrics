...
|------|------------------|
| [Trust domain](/docs/#trust-domain) | Inbound identity plane (`td`) |
| [Audience](/docs/#audience) | Outbound target (`aud`) |
| [Token exchange](/docs/#token-exchange) | Credential in → WIMSE JWT out |
| [Revocation](/docs/#revocation--kill-switch) | Immediate deny for compromised IDs |
| [PEP](/docs/#pep-policy-enforcement-point) | Starfly at runtime |
| [UTC](/docs/#utc-universal-tool-calling-layer) | One verifier, many protocols |
...
Deep dive: [trust domains](/docs/concepts/trust-domains/).
...
- MCP binds `aud` to one tool; using it elsewhere is a [confused deputy](/docs/integrators/mcp/).
...
Starfly **issues** WIMSE; SPIFFE SVIDs, K8s tokens, and IdP tokens are common **inputs** to exchange — not alternate WIMSE implementations. See [credential patterns](/docs/integrators/credential-patterns/).

## Token exchange

RFC 8693 at `POST /v1/exchange/token`. Trade a platform credential (K8s SA, OIDC, SPIFFE, stub JWT in dev) for a WIMSE JWT.

Guide: [token exchange integrator](/docs/integrators/token-exchange/) · Concepts: [exchange](/docs/concepts/exchange/).
...
Concepts: [revocation](/docs/concepts/revocation/) · Try: `./sandbox/run.sh revocation`
...
Guide: [MCP security](/docs/integrators/mcp/) · Code: [`pkg/mcp/`](https://github.com/raygj/project-starfly-fabrics/tree/main/pkg/mcp)
...
Guide: [UTC](/docs/integrators/utc/) · Code: [`pkg/toolcall/`](https://github.com/raygj/project-starfly-fabrics/tree/main/pkg/toolcall)
...
Guide: [Starfly Graph](/docs/integrators/starfly-graph/)
