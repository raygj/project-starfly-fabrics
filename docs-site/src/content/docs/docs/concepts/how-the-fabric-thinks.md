...
|-------|------|------------------------|
| [CALM Forge](/docs/ecosystem/calm-forge/) | Intent declared | `Workload`, `Capability`, `TrustDomain`, `Placement` |
| [Starfly Graph](/docs/integrators/starfly-graph/) | Fabric observed | `Agent`, `Exchange`, `ToolCall`, `Delegation` |
...
[Reasoner](/docs/ecosystem/reasoner/) sits on that seam: *does what happened match what was declared?*
...
Vault implements this with **one Sentinel rule** and an HTTP call to OPA — not thousands of per-agent policies. Guardrails (TTL ceiling, delegation depth cap, scope monotonicity) stay in Sentinel as absolute boundaries; intent and context live in Rego. See [Vault as PEP with external PDP](/docs/integrators/vault-pep-pdp/).

See [Credential patterns](/docs/integrators/credential-patterns/) for the full upstream composition with Starfly.
...
You can deploy only Starfly and get a correct PEP: exchange, revocation, MCP verify. The autonomic arc completes when you add satellites — [CALM Forge](/docs/ecosystem/calm-forge/) for intent, [Reasoner](/docs/ecosystem/reasoner/) for coherence, [Reflector](/docs/ecosystem/reflector/) for wire truth — all **async**, none on the exchange or revocation hot paths.

**Starfly alone is enough.** The full loop is optional depth for teams ready to close intent → enforcement → reconciliation.

## Related

- [Ecosystem overview](/docs/ecosystem/) — fabric map
- [Reasoner](/docs/ecosystem/reasoner/) — coherence engine
- [CALM Forge](/docs/ecosystem/calm-forge/) — design-time graph satellite
- [Documentation voice](/docs/voice/) — how we write for integrators
