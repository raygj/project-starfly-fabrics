...
|---------|-----------------|--------------|--------|
| [SPIFFE / SPIRE](/docs/integrators/#spiffe--spire) | Workload identity (attestation) | WIMSE + policy + kill switch | Shipped |
| [Kubernetes SA](/docs/integrators/#kubernetes-service-account) | Pod identity (platform JWT) | WIMSE + policy + kill switch | Shipped |
| [Vault OIDC plugin](/docs/integrators/#vault-oidc-plugin-preview) | External IdP token (Azure, Okta, …) | Optional WIMSE layer | Preview |
| [Cloud workload identity](/docs/integrators/#cloud-workload-identity) | AWS / GCP / Azure runtime cred | WIMSE + policy + kill switch | Shipped |
...
Pair with [MCP security](/docs/mcp/) when `audience` is a tool `resource_uri`.
...
See the full pattern: [Vault as PEP with external PDP](/docs/vault-pep-pdp/).
...
FIAM signaling (preflight + claim enrichment) sits **before** the IdP request — shaping PDP input on the vault path, same determinism boundary as [Vault PEP → OPA](/docs/vault-pep-pdp/).
...
| Goal | Doc |
|------|-----|
| Tool-scoped token | [MCP security](/docs/mcp/) |
| Multi-protocol middleware | [UTC](/docs/utc/) |
| Kill compromised cred | [Revocation](/concepts/revocation/) |

## Related

- [Vault PEP / PDP](/docs/vault-pep-pdp/) — Sentinel → OPA at issuance
- [Ecosystem overview](/ecosystem/) — full fabric map
- [Token exchange](/docs/token-exchange/) — wire-up guide
