...
|---------|-----------------|--------------|--------|
| [SPIFFE / SPIRE](/docs/docs/integrators/credential-patterns/#spiffe--spire) | Workload identity (attestation) | WIMSE + policy + kill switch | Shipped |
| [Kubernetes SA](/docs/docs/integrators/credential-patterns/#kubernetes-service-account) | Pod identity (platform JWT) | WIMSE + policy + kill switch | Shipped |
| [Vault OIDC plugin](/docs/docs/integrators/credential-patterns/#vault-oidc-plugin-preview) | External IdP token (Azure, Okta, …) | Optional WIMSE layer | Preview |
| [Cloud workload identity](/docs/docs/integrators/credential-patterns/#cloud-workload-identity) | AWS / GCP / Azure runtime cred | WIMSE + policy + kill switch | Shipped |
...
Trust domains often mirror SPIFFE trust domains (`spiffe://production.example.com`). See [trust domains](/docs/docs/concepts/trust-domains/).
...
Pair with [MCP security](/docs/docs/integrators/mcp/) when `audience` is a tool `resource_uri`.
...
See the full pattern: [Vault as PEP with external PDP](/docs/docs/integrators/vault-pep-pdp/).
...
FIAM signaling (preflight + claim enrichment) sits **before** the IdP request — shaping PDP input on the vault path, same determinism boundary as [Vault PEP → OPA](/docs/docs/integrators/vault-pep-pdp/).
...
| Goal | Doc |
|------|-----|
| Tool-scoped token | [MCP security](/docs/docs/integrators/mcp/) |
| Multi-protocol middleware | [UTC](/docs/docs/integrators/utc/) |
| Kill compromised cred | [Revocation](/docs/docs/concepts/revocation/) |

## Related

- [Vault PEP / PDP](/docs/docs/integrators/vault-pep-pdp/) — Sentinel → OPA at issuance
- [Ecosystem overview](/docs/docs/ecosystem/) — full fabric map
- [Token exchange](/docs/docs/integrators/token-exchange/) — wire-up guide
- [Glossary: WIMSE JWT](/docs/docs/glossary/#wimse-jwt)
