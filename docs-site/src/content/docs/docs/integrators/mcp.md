...
- **Visible in ops** — denials show on the [MCP Security dashboard tab](/docs/dashboard/) and in audit.
...
For multi-protocol tool servers, add [UTC](/docs/utc/) middleware in front of handlers; PEP-side registration and verify stay the same.
...
The `audience` **must** equal the tool's `resource_uri`. See [token exchange](/docs/token-exchange/) for credential types and fields.
...
- [Token exchange](/docs/token-exchange/)
- [UTC](/docs/utc/) — same identity on non-MCP wire shapes
