...
This is the upstream seam in [credential patterns](/docs/credential-patterns/): Vault attests and issues; Starfly mints WIMSE downstream when the fabric needs revocation, audience binding, and federation.
...
The [OIDC plugin](/docs/credential-patterns/#vault-oidc-plugin-preview) adds IdP bridge mode when downstream APIs require Azure/Okta/Google tokens. FIAM preflight runs **before** the IdP request — role-level context gates, analogous to PDP input shaping.
...
- [Credential patterns](/docs/credential-patterns/) — full upstream composition
- [Token exchange](/docs/token-exchange/) — Starfly PEP downstream
