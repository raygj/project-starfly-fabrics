...
1. Exchange a platform credential for a WIMSE JWT — [token exchange](/docs/token-exchange/).
2. Register the tool on the PEP (resource URI + allowed protocols) — [MCP security](/docs/mcp/) covers PEP-side registration.
...
- [MCP security](/docs/mcp/) — register and verify on the PEP
